...
Dynamic fetch only. Never hardcode via API.
...
- `gap_to_leader` for lapped = string `"+1 LAP"` / `"+2 LAPS"`
- `interval` for lapped = string `"LAP"` (not numeric)
- `raceControlRef.current` must be populated in `fetchRaceData` (not only `fetchRaceControl`)
...
- Surgical changes only. Ask before extensive refactor.
