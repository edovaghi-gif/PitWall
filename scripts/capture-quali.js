...
if (!SESSION_KEY) {
  console.error('Usage: node scripts/capture-quali.js <session_key>');
  process.exit(1);
}

const OUTPUT_DIR = path.join(__dirname, '..', 'captured-data', `quali_${SESSION_KEY}`);
...
async function captureSnapshot() {
  ...
  try {
    const [laps, raceControl, drivers, position] = await Promise.all([
    ...
    console.log(`  Saved: ${filename} (laps: ${laps.length}, rc: ${raceControl.length})`);
    snapshotIndex++;
  } catch (err) {
